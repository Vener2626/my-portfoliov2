...
export default function Skills() {
  return (
    <section id="skills" className="px-6 sm:px-14 py-28 max-w-3xl">
      <Reveal>
        <span className="stamp mb-6">Entry 005 — Skills</span>
      </Reveal>
      <Reveal delay={0.05}>
      ...
      <div className="grid sm:grid-cols-2 gap-x-10 gap-y-10">
        {groups.map((g, i) => (
          <Reveal key={g.label} delay={0.06 * i}>
            <h3 className="font-mono text-xs tracking-widest uppercase text-azure mb-4">
              {g.label}
            </h3>
            <ul className="flex flex-wrap gap-2">
              {g.items.map((item) => (
                <li
                  key={item}
                  className="text-sm text-paper border border-ink-line px-3 py-1.5 hover:border-azure hover:text-azure transition-colors"
                >
                  {item}
                </li>
              ))}
